...
fun computeTransformationMatrix(tripletOther: List<Star>, tripletReference: List<Star>): Matrix? {
    ...
    for (i in 0..2) {
        val x = tripletOther[i].x.toDouble()
        val y = tripletOther[i].y.toDouble()
        val xPrime = tripletReference[i].x.toDouble()
        val yPrime = tripletReference[i].y.toDouble()
...
fun applyTransformationToImage(inputImage: Image, transformationMatrix: Matrix): Image {
    ...
    for (yPrime in 0 until height) {
        for (xPrime in 0 until width) {
            ...
            outputVector[0, 0] = xPrime.toDouble()
            outputVector[1, 0] = yPrime.toDouble()
