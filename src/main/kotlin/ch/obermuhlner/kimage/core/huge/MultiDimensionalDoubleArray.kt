...
abstract class MultiDimensionalDoubleArray(private vararg val dimensions: Int) : Closeable {

    abstract operator fun get(index: Long): Double
    abstract operator fun set(index: Long, value: Double)

    val size: Long
        get() = dimensions.map(Int::toLong).reduce(Long::times)

    operator fun get(index0: Int, index1: Int): Double {
    ...
    operator fun get(index0: Int, index1: Int, index2: Int): Double {
        require(dimensions.size >= 3)
        val index = index0.toLong() + index1.toLong() * dimensions[0] + index2.toLong() * (dimensions[0] * dimensions[1])
        return get(index)
    }
    operator fun set(index0: Int, index1: Int, index2: Int, value: Double) {
        require(dimensions.size >= 3)
        val index = index0.toLong() + index1.toLong() * dimensions[0] + index2.toLong() * (dimensions[0] * dimensions[1])
        set(index, value)
    }

    operator fun get(index0: Int, index1: Int, index2: Int, index3: Int): Double {
    ...
    operator fun set(index0: Int, index1: Int, index2: Int, index3: Int, value: Double) {
        require(dimensions.size >= 4)
        val index = index0.toLong() + index1.toLong() * dimensions[0] + index2.toLong() * (dimensions[0] * dimensions[1]) + index3.toLong() * (dimensions[0] * dimensions[1] * dimensions[2])
        set(index, value)
    }
